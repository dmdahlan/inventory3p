...
class Master_driver extends BaseController
{
    ...
    public function ajax_list()
    {
        ...
        foreach ($list as $r) {
            ...
            $row[] = $r->alamat;
            $row[] = '
                    <a class="btn btn-warning btn-xs" href="javascript:void(0)" title="Edit" onclick="edit_driver(' . "'" . $r->id_driver . "'" . ')">Edit</a>
                    <a class="btn btn-danger btn-xs" href="javascript:void(0)" title="Hapus" onclick="hapus_driver(' . "'" . $r->id_driver . "'" . ')">Hapus</a>
                    ';
            $data[] = $row;
        }
        $output = array(
    ...
    public function update_driver()
    {
        ...
        $data = [
            'id_driver'       => $this->request->getPost('id'),
            'nama'            => $this->request->getPost('nama'),
            'nohp'            => $this->request->getPost('nohp'),
            'alamat'          => $this->request->getPost('alamat')
        ];
    ...
    public function _getRulesValidation($method = null)
    {
        if ($method == 'save') {
            ...
            $nama         = 'required|is_unique[master_driver.nama,id_driver,{id}]';
        }
        $rulesValidation = [
            'nama' => [
                'rules' => $nama,
