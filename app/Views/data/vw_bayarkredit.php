...
<section class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-6">
                    <h5 class="text-dark">Pembayaran Kredit</h5>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="#">Home</a></li>
                        <li class="breadcrumb-item"><a href="#">Pembelian</a></li>
                        <li class="breadcrumb-item active">Pembayaran Kredit</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
...
<script type="text/javascript">
    ...
    function tambah_bayar(id) {
        ...
        $('#modal-title').text('Tambah pembayaran kredit');
        $('#btnSavekredit').text('Save');

        $.ajax({
            url: '<?= site_url('pembelian_bayarkredit/edit/'); ?>' + id,
            type: 'GET',
            dataType: 'JSON',
    ...
    function simpan() {
        if (method == 'save') {
            url = '<?= site_url('pembelian_bayarkredit/save') ?>';
        } else {
            url = '<?= site_url('pembelian_bayarkredit/update') ?>';
        }
        $.ajax({
            url: url,
            type: 'POST',
    ...
    function edit_bayar(id) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $('#md-form-kredit').modal('show');
                $('#modal-title').text('Edit pembayaran kredit');
            },
            error: function(jqXHR, textStatus, errorThrown) {
                alert('Error!');
            }
    ...
    function hapus_kredit(id) {
        swal.fire({
            ...
            if (willDelete.value) {
                $.ajax({
                    url: "<?php echo site_url('pembelian_bayarkredit/delete') ?>/" + id,
                    type: "POST",
                    dataType: "JSON",
                    success: function(data) {
